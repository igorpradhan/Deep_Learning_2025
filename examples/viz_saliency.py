...
if __name__ == "__main__":
    print(f"Starting Memory-Optimized Visualization on {DEVICE}...")
    
    # 1. Load Data
    if not os.path.exists(PARQUET_PATH):
        if os.path.exists(f"./work_dir/DL_2025 copy/{PARQUET_PATH}"):
            PARQUET_PATH = f"./work_dir/DL_2025 copy/{PARQUET_PATH}"
        elif os.path.exists("work_dir/DL_2025 copy/GM12878_RNAPII_64k_strict.parquet"):
             PARQUET_PATH = "work_dir/DL_2025 copy/GM12878_RNAPII_64k_strict.parquet"
    
    print(f"Reading {PARQUET_PATH}...")
    df = pd.read_parquet(PARQUET_PATH)
    df = df[df['chrom'] == 'chr1'].reset_index(drop=True)
    print(f"Filtered dataset to {len(df)} samples (chr1 only).")
    ...
    # 3. Search Loop
    print("🔍 Searching for positive examples...")
    found_count = 0
    target_count = 3
    ...
    for i in range(len(df)):
        ...
        try:
            ...
            # Lowered confidence check to 0.65 to find examples faster
            if conf > 0.65:
                print(f"Found Sample #{i} (Confidence: {conf:.4f})")
                e_range = (int(row['enh_rel_start']), int(row['enh_rel_end']))
                p_range = (int(row['prom_rel_start']), int(row['prom_rel_end']))
                plot_importance(scores[0], seq, e_range, p_range, i)
                found_count += 1
                if found_count >= target_count:
                    print("Finished generating 3 examples.")
                    break
        
        except RuntimeError as e:
            if "out of memory" in str(e):
                print(f"⚠️ OOM on sample {i}, cleaning up...")
                torch.cuda.empty_cache()
            else:
